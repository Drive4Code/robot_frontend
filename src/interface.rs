...
pub fn nico_ai() -> Html {
    ...
    fn rock(robot: &mut impl Runnable, world: &mut World) {
        //check if backpack is full, if so go to street
        recharge(robot);
        let v1  = robot.get_backpack().get_contents().get(&Content::Rock(0)).unwrap();
        let v2 = robot.get_backpack().get_contents().get(&Content::Garbage(0)).unwrap();
        let total = v1+v2;
        if total == 20usize {
            call(robot, world, "street");
        }
        //find rocks
        match TomTom::go_to_tile(robot, world, false, None, Some(PlainContent::Rock)) {
            Ok(path) => {
                ...
                match bessie::bessie::road_paving_machine(
                    ...
                    Ok(b) => call(robot, world, "street"),
                    Err(e) => call(robot, world, "street"),
                };
            },
            Err(e) =>
                {
                    //if rocks weren't found, try garbage
                    match TomTom::go_to_tile(robot, world, false, None, Some(PlainContent::Garbage)) {
                        ...
                        Ok(path) => {
                            ...
                            destroy(robot, world, Direction::Down);
                            //see how much garbage in the backpack
                            for (key, value) in robot.get_backpack().get_contents().clone(){
                                if key == Content::Garbage(0){
                                    if value >= 10 {
                                        collect_garbage(robot, world);
                                    } else {
                                        //check amount of rock to decided weather to pave or search for rocks
                                        if key == Content::Rock(0) {
                                            if value > 0 {
                                                call(robot, world, "street");
                                            } else {
                                                call(robot, world, "rock");
                                            }
                                        }
                                    }
                                }
                        ...
                        _ => random_movement(robot, world, 'r'),
                    };
                },
            _ => random_movement(robot, world, 'r')
        };
    ...
    fn street_paving_mode(robot: &mut impl Runnable, world: &mut World) {
        println!("Let's build some roads");
        recharge(robot);
        //try paving in each direction, default case to handle if destroy() finds content
        match destroy(robot, world, Direction::Right) {
                        ...
                        match destroy(robot, world, Direction::Left) {
                            Err(e) => {
                                ...
                                match bessie::bessie::road_paving_machine(
                                    ...
                                    bessie::bessie::State::MakeRoad) {
                                    Err(RpmError::CannotPlaceHere) => {
                                        match destroy(robot, world, Direction::Up) {
                                            Err(e) => {
                                                recover_oob(robot, world, Direction::Up);
                                                go(robot, world, Direction::Down);
                                                match bessie::bessie::road_paving_machine(
                                                    robot,
                                                    world,
                                                    ...
                                                    Err(RpmError::CannotPlaceHere) => {
                                                        match destroy(robot, world, Direction::Down) {
                                                            ...
                                                            _ => content_handling(robot, world, &Direction::Down),
                                                        };
                                                    },
                                                    _ => {
                                                        return;
                                            ...
                                            _ => content_handling(robot, world, &Direction::Up),
                                        };
                                    },
                                    _ => {
                                        return;
                            ...
                            _ => content_handling(robot, world, &Direction::Left),
                        };
                    },
                    _ => {
                        return;
            ...
            _ => content_handling(robot, world, &Direction::Right),
        };
    }
    ...
    fn garbage(robot: &mut impl Runnable, world: &mut World) {
        ...
        //when garbage is found, try to keep looking for garbage
        recharge(robot);
        match TomTom::go_to_tile(robot, world, false, None, Some(PlainContent::Garbage)) {
            Ok(path) => {
                println!("Garbage was found!");
                //check amounts of rocks to possibly pave
                for (k, v) in robot.get_backpack().get_contents().clone(){
                    if k == Content::Rock(0){
                        if v >= 10{
                            go(robot, world, Direction::Right);
                ...
                //check amount of garbage to possibly try to find a bin
                for (key, value) in robot.get_backpack().get_contents().clone() {
                    if key == Content::Garbage(0) {
                        if value == 10 {
    ...
    fn collect_garbage(robot: &mut impl Runnable, world: &mut World) {
        //create tracker for task : throw garbage
                ...
                match throw_garbage(robot, world, Content::Garbage(0), 10, Direction::Up, &mut tracker) {
                    Ok(..) => {
                        if tracker.get_completed_number() == 1 {
                            ...
                            //check tiles around a bin for another bin
                            recharge(robot);
                            go(robot, world, Direction::Down);
                            match throw_garbage(robot, world, Content::Garbage(0), 10, Direction::Down, &mut tracker){
                                Ok(..) => return,
                                Err(..) => {
                                    go(robot, world, Direction::Right);
                                    go(robot, world, Direction::Up);
                    ...
                    Err(..) => {
                        //if backpack contains rocks, pave
                        for (k, v) in robot.get_backpack().get_contents().clone(){
                            if k == Content::Rock(0){
                                if v != 0 {
                                    call(robot, world, "street");
    ...
    //logic to handle content found by desotry() interface
    fn content_handling(robot: &mut impl Runnable, world: &mut World, direction: &Direction){
        match *direction {
            Direction::Down => {
                go(robot, world, Direction::Up);
                destroy(robot, world, Direction::Down);
            ...
            Direction::Left => {
                go(robot, world, Direction::Right);
                destroy(robot, world, Direction::Left);
            },
            Direction::Right => {
                go(robot, world, Direction::Left);
                destroy(robot, world, Direction::Right);
            },
            _ => return
        };
        for (key, value) in robot.get_backpack().get_contents().clone() {
    ...
    //function dispatch
    fn call(robot: &mut impl Runnable, world: &mut World, key: &str) {
        recharge(robot);
        match key {
            "rock" => rock(robot, world),
            "street" => street_paving_mode(robot, world),
            "garbage" => garbage(robot, world),
            _ => println!("invalid function call"),
        }
    }

    //dynamo tool shortcut
    fn recharge(robot: &mut impl Runnable) {
        *robot.get_energy_mut() = Dynamo::update_energy();
    }

    //border case when tomtom tool cannot find anything
    ...
    //recover from closeness to borders
